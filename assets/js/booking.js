...
(function () {
  ...
  // ── Validation ─────────────────────────────────────────────
  // Validates exactly these 6 fields. age / gender / doctor are
  // collected in the payload but do NOT block form submission.
  ...
  function validateForm() {
    ...
    // Phone format check (only runs when phone is non-empty; presence already checked above)
    const phoneField = document.getElementById("phone");
    if (phoneField && phoneField.value.trim()) {
      if (!validatePhone()) valid = false;
    }

    // gender and doctor are optional — no radio validation here
  ...
  function buildPayload() {
    ...
    return {
      ...
      // phone sent exactly as the patient typed it — no trimming
